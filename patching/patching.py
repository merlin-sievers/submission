...
class Patching:
    def __init__(self, patching_config):
        ...
        self.cfge_vuln_specific = self.project_vuln.analyses.CFGEmulated(keep_state=True, context_sensitivity_level=0, state_add_options=option, starts=[self.entry_point_vuln], call_depth=2)
        ...
        # self.patching_config.functionName ="state_machine"
        ...
        self.cfge_patch_specific = self.project_patch.analyses.CFGEmulated(keep_state=True, context_sensitivity_level=0, state_add_options=option, starts=[self.entry_point_patch], call_depth=2)


        print("\n\t Starting to analyze the patch Program DDG...")
        self.ddg_patch_specific = self.project_patch.analyses.DDG(cfg=self.cfge_patch_specific, start=self.entry_point_patch, call_depth=1)
    ...
    def patch(self, binary_fname):
        ...
        jump_back_address = min([match for match in perfect_matches.match_old_address if match > self.code_block_end.addr], default=None)

        # TODO: Check if whole function should be patched or not
        # patch_start_address_of_patch = min(patch_blocks)
        patch_start_address_of_patch = self.entry_point_patch
        ...
        # Jump to new Memory
        print("\n\t " + str(self.start_address_of_patch), self.patch_code_block_end.addr - self.patch_code_block_start.addr)
        ...
        self.jump_to_new_memory(self.start_address_of_patch, new_memory_address, patch_start_address_of_patch)
        ...
        if jump_back_address is not None:
            ...
            patches = InlinePatch(self.writing_address, "bl " + target_address_str, is_thumb=self.code_block_start.thumb)
            self.patches.append(patches)
...
#       print("\n\t IsInShiftList " + "%s %s", shifts.get(shifts.size() - 1).start, shifts.get(shifts.size() - 1).end);
    ...
    def handle_reference_without_ddg(self, instruction_patch, reference):
        ...
        cfge_help = self.project_patch.analyses.CFGEmulated(keep_state=True, context_sensitivity_level=0, state_add_options=angr.sim_options.refs, starts=[block.addr], call_depth=0)
    ...
    def add_control_flow_jump_reference(self, instruction_patch, reference, matched_refs):
        ...
        if function is None:
            ...
            cfge_cfj_specific = self.project_patch.analyses.CFGEmulated(keep_state=True, context_sensitivity_level=0, state_add_options=angr.sim_options.refs, starts=[reference.toAddr], call_depth=2)
            ...
            self.new_added_function[function.addr] = self.writing_address
            while block_address in function.block_addrs_set:
                ...
                for ins in block.capstone.insns:
                    if ins.address <= function_end:
                        ...
                        reference = self.get_references_from_instruction(ins, new_refs,
                                                                         block.thumb)
