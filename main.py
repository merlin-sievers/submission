...
for r in results:
    ...
    if config.functionName is not None:
        try:
            ...
            logging.log(logging.INFO, "Patching completed successfully for binary_path: %s functionName: %s", config.binary_path, config.functionName)
            signal.alarm(0)
...
i = 1
while i <= 1:
    config = Config("firmware-test.properties", str(i))
#
