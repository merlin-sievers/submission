...
def DetourBackend(filename, data_fallback=None, base_address=None, try_pdf_removal=True, try_reuse_unused_space=False, replace_note_segment=False, try_without_cfg=False, variant=None):
    with open(filename, "rb") as f:
        ...
        if start_bytes.startswith(b"\x7fCGC"):
            detourbackendclass = getattr(importlib.import_module("patcherexMM.backends.detourbackends.cgc"), "DetourBackendCgc")
            ...
            if start_bytes.startswith(b"\x03\x00", 0x12): # EM_386
                detourbackendclass = getattr(importlib.import_module("patcherexMM.backends.detourbackends.i386"), "DetourBackendi386")
            elif start_bytes.startswith(b"\x3e\x00", 0x12): # EM_X86_64
                detourbackendclass = getattr(importlib.import_module("patcherexMM.backends.detourbackends.i386"), "DetourBackendi386")
            elif start_bytes.startswith(b"\xb7\x00", 0x12): # EM_AARCH64
                detourbackendclass = getattr(importlib.import_module("patcherexMM.backends.detourbackends.aarch64"), "DetourBackendAarch64")
            elif start_bytes.startswith(b"\x28\x00", 0x12): # EM_ARM
                if variant == "stm32":
                        detourbackendclass = getattr(importlib.import_module("patcherexMM.backends.detourbackends.arm_stm32"), "DetourBackendArmStm32")
                else:
                    detourbackendclass = getattr(importlib.import_module("patcherexMM.backends.detourbackends.arm"), "DetourBackendArm")
            elif start_bytes.startswith(b"\x08\x00", 0x12) or \
                 start_bytes.startswith(b"\x00\x08", 0x12): # EM_MIPS
                detourbackendclass = getattr(importlib.import_module("patcherexMM.backends.detourbackends.mips"), "DetourBackendMips")
                ...
                detourbackendclass = getattr(importlib.import_module("patcherexMM.backends.detourbackends.ppc"), "DetourBackendPpc")
            elif start_bytes.startswith(b"\x53\x00", 0x12):
                detourbackendclass = getattr(importlib.import_module("patcherexMM.backends.detourbackends.avr"), "DetourBackendAVR")
            else:
