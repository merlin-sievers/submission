...
class RunPatcherexHandler(idaapi.action_handler_t):
    ...
    def menu_activate(arg):
        ...
        if form.Execute():
            ...
            config_file.close()
            print "Calling patcherexMM . . ."
            popen_and_call(RunPatcherexHandler.patcherex_finish,
                           {"args": ["patcherexMM", "-c", config_file.name, "--json", "single", idaapi.get_input_file_path(), file_name]})
    ...
    def finish_populating_tform_popup(self, form, popup):
        if idaapi.get_tform_title(form) == "Patcherex":
            for act in self.acts:
                idaapi.attach_action_to_popup(form, popup, "patcherexMM:" + act, None)
