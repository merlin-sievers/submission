...
while read patch; do 
    ...
    ./configure --host=arm-linux-gnueabihf CC=arm-linux-gnueabihf-gcc AR=arm-linux-gnueabihf-ar RANLIB=arm-linux-gnueabihf-ranlib CFLAGS='-DMAGMA_ENABLE_FIXES -g -O2' --prefix="$TARGET/$new_dir/fixed" --enable-shared > /dev/null
    make > /dev/null
    ...
    ./configure --host=arm-linux-gnueabihf CC=arm-linux-gnueabihf-gcc AR=arm-linux-gnueabihf-ar RANLIB=arm-linux-gnueabihf-ranlib CFLAGS='-g -02'  --prefix="$TARGET/$new_dir/vuln" --enable-shared > /dev/null
