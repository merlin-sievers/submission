...
export LD_LIBRARY_PATH=/home/jaenich/Libraries/libjpeg-turbo-2.0.3:/home/jaenich/Magma/magma/targets/openssl/repo/vuln/

# Set the path to the fuzzer binary
fuzzer_binary="fuzz/asn1"

# Loop through all input files matching PNG00*
echo "Start Testing ..."
for input_file in "${input_dir}"*MAE*; do
...
output=$("$fuzzer_binary" < "$input_file" 2>&1)
        ...
        # Check if the output contains "REACHED" and "PNG"
            ...
            echo "Output contains 'REACHED' and 'SSL':"
            echo "$output"
            ...
            export LD_LIBRARY_PATH=/home/jaenich/Libraries/libjpeg-turbo-2.0.3:/home/jaenich/Magma/magma/targets/openssl/repo/fixed/:/usr/local/lib/zlib/lib/:/home/jaenich/Libraries/lib
            output=$("$fuzzer_binary" < "$input_file" 2>&1)
